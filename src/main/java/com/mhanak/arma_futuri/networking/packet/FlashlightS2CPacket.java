...
public class FlashlightS2CPacket {
    public static void recieve(MinecraftClient client, ClientPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        if (player != null) {
            //ArmaFuturiMod.LOGGER.info("S2C set the flashlight to {}", enabled);
            //ArmorData.setFlashlightActive((IEntityDataSaver) client.player, enabled);
