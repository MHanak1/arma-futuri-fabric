...
public class JetpackC2SPacket {
    public static void recieve(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        ArmaFuturiMod.LOGGER.debug("C2S set the jetpack to {}", enabled);
        ...
        ArmaFuturiMod.LOGGER.debug("C2S received a jetpack packet from {}", player.getUuid());

        for (PlayerEntity other_player : player.getWorld().getPlayers()){
            ServerPlayNetworking.send((ServerPlayerEntity) other_player, ModPackets.JETPACK_SYNC_ID, buffer);
